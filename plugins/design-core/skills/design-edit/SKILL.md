...
- **Don't add features the user didn't ask for.** When the user says "X needs to be included," that does NOT mean "X needs its own dedicated page / panel / screen." Default to folding X into existing structure. If unsure, ASK before expanding scope. Real-world failure mode: user said "notification permission needs to be included" and the response was to add a 5th onboarding page when notification permission should have folded into an existing page.
- **Reuse a single puppeteer browser session for a chain of renders.** Do NOT `launch / new_page / navigate / screenshot / close_browser` per iteration. Launch once at the start of the work, navigate + evaluate + screenshot N times, close at the end. Per-launch overhead is significant and the close-each-time pattern produces nothing of value beyond what reuse gives you.
- **For 2+ option design decisions, use the AskUserQuestion tool with previews.** Dumping option lists in chat is slow and error-prone. The interactive picker with ASCII/screenshot previews settles a design call in one round-trip. Reserve plain-text option lists for binary or low-stakes choices.
- **Visible gap ≠ CSS margin.** Line-height adds half-leading on top + bottom of every text line. When the spec calls for "30 px between blocks of text," CSS `margin: 30px` produces ~`30 + 2 × half-leading` visually (≈ 39 px at font-size 22 / line-height 1.4). For visible 30 px, use ~21 px CSS. For text-to-input edges (which have no leading on one side), use ~26 px. Document the math so the next reader doesn't relearn it.
- **SVG silhouette extraction: alpha threshold ≤ 10, not 50.** Anti-aliased edges of any vector shape have alpha 1–50. A 50 threshold cuts off the soft edge and gives an under-traced silhouette. When extracting outlines via canvas pixel-scan, default to 5–10 and verify the traced line visually before using it.
